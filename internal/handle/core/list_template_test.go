...
func TestBuildBreadcrumbsLinksAncestorDirectories(t *testing.T) {
	crumbs := buildBreadcrumbs("projects/design")

	if len(crumbs) != 3 {
		t.Fatalf("len(crumbs) = %d, want 3: %+v", len(crumbs), crumbs)
	}
	if crumbs[0] != (pathCrumb{Name: "根目录", URL: "/file/?action=view"}) {
		t.Fatalf("root crumb = %+v", crumbs[0])
	}
	if crumbs[1] != (pathCrumb{Name: "projects", URL: "/file/projects/?action=view"}) {
		t.Fatalf("projects crumb = %+v", crumbs[1])
	}
	if crumbs[2] != (pathCrumb{Name: "design", URL: "/file/projects/design/?action=view", Current: true}) {
...
func TestGetFileInfosSortsDirectoriesBeforeFiles(t *testing.T) {
	...
	files := getFileInfos(nil, root, root)
