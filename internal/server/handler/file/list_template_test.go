...
func TestListTemplateRendersPolishedFileBrowserControls(t *testing.T) {
	...
	assertContains(t, html, `data-upload-status role="status"`)
	assertContains(t, html, `data-upload-progress`)
	assertContains(t, html, `data-upload-progress-bar`)
	assertContains(t, html, `XMLHttpRequest`)
	assertContains(t, html, `xhr.upload.addEventListener("progress"`)
	assertContains(t, html, `window.setTimeout(function () { window.location.reload(); }, 700)`)
