...
var (
	Output     io.Writer = eutil.If[io.Writer](runtime.GOOS == "windows", color.Output, os.Stderr)
	SessionVal string    = eutil.RandString(64)
...
func setDefault() error {
	viper.Set("server.port", define.Port)
	viper.Set("server.allow_upload", define.IsAllowUpload)
	viper.Set("server.max_level", define.MaxLevel)
