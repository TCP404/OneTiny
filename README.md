...
- [构建系统](docs/build.md)：本地和 CI 构建入口、artifact 命名和 release contract。

## [使用说明]
可从本仓库的 [Release](https://github.com/tcp404/OneTiny/releases/latest) 下载对应版本。

当前 release 产物分为两类：

- CLI：`onetiny-cli-<platform>.zip`
- GUI：`onetiny-gui-<platform>.zip`

其中 `<platform>` 目前包括 `linux-x64`、`windows-x64`、`darwin-x64`、`darwin-arm64` 等 release 构建目标。具体可用产物以最新 Release 页面为准。
...
**Linux**、**Mac**：
```bash
$ curl -LJ https://github.com/tcp404/OneTiny/releases/latest/download/onetiny-cli-linux-x64.zip -o onetiny-cli.zip
$ unzip onetiny-cli.zip
$ chmod u+x ./onetiny-cli
$ sudo mv onetiny-cli /usr/local/bin/onetiny
```

Mac 用户请将示例中的 `onetiny-cli-linux-x64.zip` 替换为对应的 `onetiny-cli-darwin-x64.zip` 或 `onetiny-cli-darwin-arm64.zip`。
...
下载最新版本的 Windows CLI 或 GUI zip，解压后运行其中的 `onetiny-cli.exe` 或 `OneTiny.exe`。
...
源码构建需要先安装 [Go](https://golang.org)、Node.js、[go-task](https://taskfile.dev/)；GUI 构建还需要 Wails v3 CLI。
